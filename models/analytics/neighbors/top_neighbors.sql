...
top_five_list as (
  ...
  --   array_agg(c.institution order by abs(b.ranking_uni - c.ranking_uni) limit 5) as neighbor_,
  ...
  select
    ...
    array_agg(c.institution order by abs(b.score_uni - c.score_uni) limit 5) as neighbor_,
  from competitors_year c
  full join benchmark_year b on b.rank_year=c.rank_year
  group by 1
  ...
  select
    ...
    array_agg(c.institution IGNORE NULLS order by abs(b.nss_overall_uni - c.nss_overall_uni) limit 5) as neighbor_,
  from competitors_year c
  full join benchmark_year b on b.rank_year=c.rank_year
  group by 1
  ...
  select
    ...
    array_agg(c.institution IGNORE NULLS order by abs(b.nss_teaching_uni - c.nss_teaching_uni) limit 5) as neighbor_,
  from competitors_year c
  full join benchmark_year b on b.rank_year=c.rank_year
  group by 1
